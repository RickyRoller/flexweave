...
description: Author Flexweave-backed game mechanics and abilities in a consumer repo by reading the integration map, using Flexweave primitives, and implementing runtime behavior. Use when a user asks to add, create, change, or debug a mechanic, ability, effect, modifier, tag, cooldown, attribute, tick, or event backed by Flexweave.
---
...
`FLEXWEAVE.md` or an installed Flexweave dependency, repair setup first by
using the `flexweave-setup` skill.

1. Load context:
   - Read `FLEXWEAVE.md`.
   - Inspect the Flexweave adoption map, runtime state owner, manual mechanics
...
2. Translate the user request into a mechanic brief:
   ...
   - Flexweave backing plan: which Flexweave primitives own identity, attributes,
   ...
   - Event consumers: which UI, death/despawn, status, or follow-on runtime
     reactions must be driven by emitted Flexweave events.
...
3. Inspect current runtime state before writing:
   - Search for existing uses of Flexweave types and stores.
...
4. Implement runtime behavior:
   ...
   - Drive observable reactions from Flexweave-emitted events/facts. UI updates,
     death/despawn, status changes, and follow-on mechanics should consume
     attribute changes or lifecycle events.
   ...
   - Preserve existing manual systems unless the user requested migration. If a
     ...
     adoption gap.
   - Keep game-specific semantics in the consumer runtime; keep Flexweave
...
6. Update `FLEXWEAVE.md` coherently if the change establishes a new Flexweave-backed
   runtime path, adoption status, or verification command:
   - Add or update an authored mechanics section for the new mechanic ids,
     runtime entry points, Flexweave primitives, and tests.
...
- If Flexweave is missing or the adoption map is absent, repair setup before
  authoring.
...
- Read `references/flexweave-primitives-overview.md` when selecting which
  Flexweave primitive should own a mechanic responsibility.
