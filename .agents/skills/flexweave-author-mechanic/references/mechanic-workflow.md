...
- Match the repo's existing ownership pattern for world/state, systems, and
  tests.
- Use Flexweave for the primitive named in the mechanic brief; keep domain-specific
  payloads and rules in the consumer runtime.
- Consume Flexweave-emitted facts for observable reactions. UI projections,
  status changes, and follow-on mechanics should be updated from
...
- Record authored mechanic ids, Flexweave primitives, runtime entry points, and
...
- Revisit `Open Decisions` after wiring. Delete or rewrite entries that are no
  longer true.
- Keep dependency and adoption status precise. Flexweave should be
