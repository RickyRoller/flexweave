...
A Flexweave-backed mechanic uses Flexweave for the runtime primitive that
...
Before editing runtime behavior, read the `FLEXWEAVE.md` Flexweave adoption map and
...
- Flexweave is installed in the owning runtime crate.
...
- Ability-backed mechanics perform their primary gameplay command through the
  Flexweave ability activation executor or hook path.
- UI projections, status changes, and other observable reactions
  consume emitted Flexweave facts/events from attributes, abilities, effects, or
  mechanics ticking.
- Existing compile/check commands for the runtime pass.
- Gameplay-facing tests or scenarios exercise the mechanic behavior.
- `FLEXWEAVE.md` records new mechanics, runtime entry points, Flexweave primitives,
...
## Flexweave Primitives

Map the mechanic to the smallest Flexweave primitive that owns the lifecycle:
